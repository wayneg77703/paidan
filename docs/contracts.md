...
INSTALL.md / INSTALL.en.md define agent-led package update, upstream CLI adaptation and uninstall. They do not rerun init. The optional local install-receipt.json is maintained by the installing agent, not emitted by the CLI; it stores ownership/comparison metadata without credentials and never constitutes delete authorization. Native configurations/data/history are preserved by default; rollback is field-scoped and refuses uncertain later edits.

The legacy skill installer leaves identical files unchanged and refuses different existing content unless its caller supplies the SHA-256 of the specifically approved old bytes. Approved replacement first makes a unique backup and checks for intervening changes. Target paths must stay within the selected host skill root and links inside that tree are refused. New-file installation uses an exclusive hard link to publish complete bytes without replacing a competing file. Init stops if configuration backup fails; a skill conflict is surfaced per host rather than overwritten. Agent-led manual copies follow the same compare/backup rules in INSTALL.md.
