...
**init key ownership** (who wins for each key across a re-init; `--yes` is a deliberate reset-to-native, not "reuse current"):

| key | interactive re-init | `--yes` | owner |
|---|---|---|---|
| `endpoints.enabled` | replaced by the answer (preselected from current) | every detected endpoint | wizard |
| `defaults.endpoint` | preselected when still enabled; replaced by the answer | first detected (omitted when none) | wizard |
| `defaults.models.<ep>` | answer replaces per endpoint; keep-current is the fallback for out-of-lineup values; discovery-empty + hand-set value asks keep-vs-native | cleared (reset to native) | wizard |
| `defaults.efforts.<ep>` | preselected when valid; replaced by the answer | cleared (native); `--effort <level>` writes only endpoints whose options include it | wizard |
| `endpoints.overrides.<ep>.bin` | kept; only an in-wizard intake adds/replaces | kept | machine |
| `dataDir` / `ttlDays` / `defaults.run_timeout_sec` | kept | kept | machine |
| removed global `defaults.model`/`defaults.effort` | stripped from legacy configs | stripped (load-time migration strip + warning, NOT a hard error) | schema |
| native agent config / credentials | read-only survey (`native_defaults`, `credential_ready`) | read-only survey | the agent's own mechanisms |

- `doctor`: endpoint detection results (every manifest listed with an `enabled` flag), versions, **drift** (installed version ∉ any manifest-declared version — the recalibration trigger), permission map status, effort options, native_preflight outcome, **native_defaults** (read-only probe: the model/effort each endpoint's own home currently carries, plus `credential_ready` for endpoints with a headless auth bundle paidan never touches — zcode's env triple or native cli-config model section; invisible credentials produce an `issues[]` repair pointer for both the bin-resolved and both-missing states), host skill install state, config/data dir paths, db status, models-cache ages, and a human-readable `issues` summary. `ok:true` only means the doctor ran — read the per-endpoint fields. This is the compatibility-matrix generator.
...
- `init`: first-run wizard. Interactive on a TTY: skipped non-detected endpoints offer an in-wizard bin-override intake (enter a path → written to `endpoints.overrides.<name>`, re-probed on the spot — model discovery included —, enablable if it resolves; empty skips; paidan config only, credentials are never asked for), then a checkbox multi-select for endpoints (space toggles, enter confirms; arrows wrap, numbers, a=toggle-all, i=invert), an arrow-key menu for the default endpoint, a **per-endpoint default-model prompt for every enabled endpoint that can take a model headless** (`model_selectable = command.model_arg !== undefined`; first choice = native, a configured value inside the lineup preselects, a value outside the lineup stays selectable via `(keep current: X)` — which is also the fallback, so enter keeps a hand-set value and native is always an explicit pick; discovery-empty with a hand-set value asks keep-vs-native; endpoints like dsh/zcode whose native config owns the model get a note instead, single-candidate endpoints are asked too — an auto-pick is a silent native override), a **per-endpoint default-effort prompt where the endpoint declares an effort block** (first choice is always `(native default)` = no config entry), and a checkbox multi-select of hosts for the skill install (already-installed hosts marked; `skills/hosts.json` lists all eight agents' user-scope skills dirs, so every agent can be both delegatee and host). stderr must be a TTY for the raw-mode widgets; otherwise the wizard falls back to line-based numeric prompts. Writes config.json; existing config is backed up first **and merged** — the wizard owns `endpoints.enabled` + the wizard-owned defaults keys (`endpoint`, `model`, `models`, `efforts`; single source `WIZARD_DEFAULTS_KEYS` in config.ts). The global `defaults.model`/`defaults.effort` fallback keys were **removed from the config schema** (2026-09-11): both poisoned endpoints without that selection surface and a surviving global silently overrode a per-endpoint native choice; `loadConfig` strips them with a warning (a legacy config keeps every verb usable — it is NOT a `CONFIG_INVALID`) and one re-init removes them from the file. Non-TTY callers get `INIT_INTERACTIVE_REQUIRED` plus current state JSON; `--yes` enables all detected endpoints, **leaves every model/effort at the endpoint's native default** (the agent's own home carries them — a paidan-side first-discovered default is an override layer that silently changes native behavior), and installs the skill into every detected host; `--yes --effort <level>` additionally applies that level to every detected endpoint whose declared options include it (the ok envelope reports `effort_applied_to` / `effort_skipped`); `--yes --hosts <names>` restricts the skill install to exactly those hosts (an agent-collected subset must be faithfully installed). The survey phase persists nothing **of the wizard's own** — model caches are written only after answers commit, so an aborted init (`INIT_ABORTED`, exit 130) and a non-TTY survey both leave the disk untouched **by the wizard**. Two engine-level side effects to know, not wizard writes: the CLI's startup reconcile can update the state/events of orphaned old runs it flags, and `doctor` creates `usage.db` (+ its directory and tables) on first use. Both are data-plane (`%APPDATA%\paidan\`), never config-plane.  The decision logic lives in `engine/init-plan.ts` (UI-free, incl. `planEndpointDefaultQuestions`) so a console GUI reuses it. After the config write, init offers to install the paidan skill (`skills/paidan/SKILL.md`, payload listed in `skills/hosts.json`) into each selected host's user-scope skills dir — per-host multi-select on a TTY, all detected hosts under `--yes`; installs are atomic copies reported as created/updated/unchanged, and only ever happen on explicit selection (invariant 4: no silent writes to an agent's native home). Each host receives its own **natively-authored variant** — `skills/hosts.json` maps every host to `skills/paidan/variants/<host>.SKILL.md` (native frontmatter per that host's spec, identical body, installed verbatim with no field stripping; `adaptSkillPayload` remains as the compatibility path for older payloads).
