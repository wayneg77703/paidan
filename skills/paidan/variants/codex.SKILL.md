---
name: paidan
description: 委派任务给本机 AI CLI agent 执行：通过 paidan CLI 把任务交给 kimi-code/codex/claude-code/zcode/opencode/omp/dsh/agy（run/get/cancel/list/models/doctor/probe），每个任务是一个持久 run，可等待、可取消、重启后可查证。需要把任务委派给本机 agent、跨 agent 调第二意见、派对抗审查或并行子任务时使用本通道——本机委派的唯一受管通道。
---
