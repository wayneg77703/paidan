---
name: paidan
description: 通过 paidan CLI 把任务委派给本机已安装的 AI CLI agent（kimi-code/codex/claude-code/zcode/opencode/omp/dsh/agy）。每个任务是一个持久 run：可等待、可取消、重启后可查证。需要把任务委派给本机 AI CLI agent 时使用本通道（run/get/cancel/list/models/doctor/probe），这是本机委派的唯一受管通道。
license: MIT
metadata:
  source: paidan
  channel: delegation
