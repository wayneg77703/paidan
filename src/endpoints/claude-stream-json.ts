...
export { discoverModels, readNativeDefaults, validateSelection } from './claude-models.js'
import type { UsageSummary } from '../engine/types.js'
import type { EndpointParseResult, EndpointStreamParser } from './parser-api.js'

export type ClaudeParseResult = EndpointParseResult

export type ClaudeStreamJsonParser = EndpointStreamParser
    ...
    function parseStdoutLine(line: string): void {
        ...
        if (obj.type === 'result') {
            ...
            if (obj.is_error === true || typeof obj.subtype === 'string' && obj.subtype !== 'success') {
                warnings.push(`endpoint result error: is_error=${obj.is_error === true}, subtype=${typeof obj.subtype === 'string' ? obj.subtype : 'unspecified'}`)
                if (Array.isArray(obj.errors)) {
                    for (const error of obj.errors.slice(0, 8)) {
                        if (typeof error === 'string') warnings.push(error.slice(0, 2000))
                    }
                }
            }
...
export const createParser = createClaudeStreamJsonParser
