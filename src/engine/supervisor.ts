...
// Windows worker registration first queries process identity (up to 10s).
// The parent must allow that bounded query plus process startup/I/O; an equal
// 10s deadline can report failure just before a healthy worker registers.
const DEFAULT_WORKER_START_TIMEOUT_MS = 20_000
...
export async function launchWorker(runId: string, opts: LaunchWorkerOptions): Promise<number> {
    ...
    const deadline = Date.now() + (opts.timeoutMs ?? DEFAULT_WORKER_START_TIMEOUT_MS)
    ...
    throw new Error(`worker did not become observable within ${opts.timeoutMs ?? DEFAULT_WORKER_START_TIMEOUT_MS}ms for ${runId}`)
}
