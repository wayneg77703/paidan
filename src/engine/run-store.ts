...
export interface CreateRunInput {
    ...
    native_selection?: RunRequest['native_selection']
    ...
    async create(input: CreateRunInput, now: string = new Date().toISOString()): Promise<CreateRunOutcome> {
        ...
        try {
            ...
            const request: RunRequest = {
                ...
                model: input.model,
                effort: input.effort,
                ...(input.selection_context ? { selection_context: input.selection_context } : {}),
                ...(input.native_selection ? { native_selection: input.native_selection } : {}),
                resume_session: input.resume_session,
                ...(input.provider_config !== undefined ? { provider_config: input.provider_config } : {}),
