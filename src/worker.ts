...
import { buildArgs, buildEnv, withPromptCwdHint } from './endpoints/invocation.js'
import { prepareEndpointRuntime, createEndpointParser, loadParserModule } from './endpoints/adapters.js'
import { EndpointRegistry, ManifestError } from './endpoints/registry.js'
...
async function main(): Promise<number> {
    ...
    try {
        ...
        const { parser, detectRefusals, readLedgerUsage, captureLedgerCursor, readExecutionSelection } = await createEndpointParser(manifest)
        ...
        if (request.selection_context) {
            ...
            const current = await mod.readNativeDefaults?.({ configBin: config.endpoints.overrides[request.endpoint]?.bin ?? null, cwd: request.cwd })
            if (current?.selection_context !== request.selection_context) {
                return fail('SELECTION_RECONFIRM_REQUIRED', `${request.endpoint} 原生配置在提交后发生变化；端点未启动。请重新查询当前通路并让用户选择。`)
            }
        }
        ...
        const runtime = await prepareEndpointRuntime(manifest, plan, buildEnv(manifest, process.env, request.mode, request.effort), request.cwd, request.provider_config)
        ...
        const selection = readExecutionSelection ? await readExecutionSelection(parsed, { env: runtime.env, expected: runtime.expected }) : undefined
        if (selection) usageNotes.push(...selection.notes)
        ...
        if (manifest.name === 'zcode' && judgment.state !== 'completed') notes.push('未自动换连接或重派。若要改用原生配置，请先向用户说明失败原因和可能已执行的操作，取得同意后再决定续接或新建任务。')
        if ((judgment.state === 'failed' || judgment.state === 'unknown') && stderrTail.trim()) {
            // Redact BEFORE truncation so a cut cannot hide a token's prefix.
            notes.push(`endpoint stderr (bounded, redacted tail): ${redactor.redactText(stderrTail).slice(-4096).trim()}`)
        }
        await store.appendEvent(runId, redactor.redactJson({
