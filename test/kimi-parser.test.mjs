...
import { createKimiPrintParser, detectKimiRefusals } from '../dist/endpoints/kimi-print.js'
...
test('model discovery uses native provider mapping and never emits credentials', () => {
    const input = {
        providers: {
            'managed:login': { oauth: { accessToken: 'secret-login' }, apiKey: '' },
            api: { apiKey: 'secret-api', baseUrl: 'https://secret-host.invalid' },
        },
        models: {
            'friendly/k3': { provider: 'managed:login', supportEfforts: ['low', 'high', 'max'], defaultEffort: 'high', overrides: { defaultEffort: 'max' } },
            'other/k3': { provider: 'api' },
        },
    }
    const found = parseKimiModelsJson(JSON.stringify(input))
    assert.equal(found.models[0].connection, 'managed:login')
    assert.deepEqual(found.models[0].effort_options, ['low', 'high', 'max'])
    assert.equal(found.models[0].default_effort, 'max')
    assert.equal(found.models[1].effort_options, null)
    assert.deepEqual(found.connections.map(c => c.auth_type), ['oauth', 'api-key'])
    assert.ok(!JSON.stringify(found).includes('secret-'))
    assert.throws(() => parseKimiModelsJson('{broken'), /invalid JSON/)
    assert.throws(() => parseKimiModelsJson('{"models":[]}'), /unknown schema/)
    assert.throws(() => parseKimiModelsJson('null'), /unknown schema/)
})
