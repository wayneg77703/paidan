...
import { createParser, detectRefusals, parseModelsOutput } from '../dist/endpoints/opencode-run.js'
...
test('parseModelsOutput: provider/model lines with connection prefix; garbage skipped honestly', () => {
    const text = 'opencode/big-pickle\nopencode-go/deepseek-v4-flash\nopencode-go/kimi-k3\n\ngarbage line\n'
    const { models, notes } = parseModelsOutput(text)
    assert.deepEqual(models, [
        { alias: 'opencode/big-pickle', connection: 'opencode' },
        { alias: 'opencode-go/deepseek-v4-flash', connection: 'opencode-go' },
        { alias: 'opencode-go/kimi-k3', connection: 'opencode-go' },
    ])
    assert.ok(notes.some((n) => n.includes('1 non-model line')))
    const empty = parseModelsOutput('')
    assert.deepEqual(empty.models, [])
    assert.ok(empty.notes.some((n) => n.includes('no parseable')))
})
