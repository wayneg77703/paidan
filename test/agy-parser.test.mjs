...
import { createParser, detectRefusals } from '../dist/endpoints/agy-print.js'
import { parseAgyModelsOutput } from '../dist/endpoints/agy-models.js'
...
test('parseAgyModelsOutput: TSV id<TAB>display-name rows (live shape 1.1.28)', () => {
    ...
    assert.deepEqual(models.map(({ alias, connection, source }) => ({ alias, connection, source })), [
        { alias: 'gemini-3.8-flash-high', connection: null, source: 'native-catalog' },
        { alias: 'gemini-3.1-pro-low', connection: null, source: 'native-catalog' },
    ])
    assert.deepEqual(models.map(m => [m.default_effort, m.effort_selectable]), [['high', false], ['low', false]])
