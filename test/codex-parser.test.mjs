...
import { createCodexExecParser, detectCodexRefusals } from '../dist/endpoints/codex-exec.js'
import { codexConfigMetadata } from '../dist/endpoints/codex-models.js'
...
test('native metadata keeps selection fields separate from provider and MCP sections', () => {
    const native = codexConfigMetadata('model="configured-model"\nmodel_provider="custom-provider"\n[model_providers.custom-provider]\nmodel="not-top-level"\n[mcp_servers.x]\nmodel="not-a-model"\n')
    assert.equal(native.values.model, 'configured-model')
    assert.equal(native.values.model_provider, 'custom-provider')
    assert.deepEqual(native.providers, ['custom-provider'])
    assert.equal(codexConfigMetadata('[model_providers.custom]\nmodel="nested"\n').values.model, undefined)
})

test('native metadata distinguishes missing defaults from configured values', () => {
    const parsed = codexConfigMetadata('model="chosen-model"\nmodel_reasoning_effort="high"\n')
    assert.equal(parsed.values.model, 'chosen-model')
    assert.equal(parsed.values.model_reasoning_effort, 'high')
    assert.equal(codexConfigMetadata('model="partial"').values.model_reasoning_effort, undefined)
    assert.deepEqual(Object.keys(codexConfigMetadata(null).values), [])
})

test('native metadata handles literal strings/comments and marks unresolved profiles', () => {
    const parsed = codexConfigMetadata("model = 'custom-model' # native choice\nmodel_provider = 'custom'\nmodel_reasoning_effort = 'low'\n")
    assert.equal(parsed.values.model, 'custom-model')
    assert.equal(parsed.values.model_provider, 'custom')
    assert.equal(parsed.values.model_reasoning_effort, 'low')
    assert.equal(parsed.unresolved, false)
    const profile = codexConfigMetadata('profile="alternate"\nmodel="base-model"\n[profiles.alternate]\nmodel="profile-model"\n')
