...
| zcode | – / – / ✅ | 设计上仅 yolo（非 yolo 档 headless 下永远等待）。桌面版不装 PATH 二进制——默认安装目录由 `detect.known_paths` 自动探测；自定义安装位置 init 向导内可直接录入 bin 路径（或手工设 `endpoints.overrides.zcode.bin`/自制 shim）。headless 还需要凭据（paidan 永不接触）：`doctor` 预检 `credential_ready` 并指路原生 `~/.zcode/cli/config.json` 的 model 段或环境变量三元组。 |
