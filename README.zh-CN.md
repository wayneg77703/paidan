...
| zcode | – / – / ✅ | 设计上仅 yolo（非 yolo 档 headless 下永远等待）。桌面版不装 PATH 二进制——默认安装目录由 `detect.known_paths` 自动探测；自定义安装位置 init 向导内可直接录入 bin 路径（或手工设 `endpoints.overrides.zcode.bin`/自制 shim）。headless 还需要用户自行开通的凭据：官方方式是 `node <安装目录>/resources/glm/zcode.cjs login`（Z.AI OAuth；无浏览器环境加 `--no-browser`）创建共享登录态——备选：env 三件套（`ZCODE_MODEL`/`ZCODE_BASE_URL`/`ANTHROPIC_API_KEY`），或把桌面端 `~/.zcode/v2/config.json` 的 `provider` 块手工同步进 `~/.zcode/cli/config.json`（桌面老用户的现有做法）。`doctor` 以 `credential_ready` 预检。 |
