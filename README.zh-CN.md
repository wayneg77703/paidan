...
| zcode | – / – / ✅ | 设计上仅 yolo（非 yolo 档 headless 下永远等待）。桌面版不装 PATH 二进制——默认安装目录由 `detect.known_paths` 自动探测；自定义安装位置 init 向导内可直接录入 bin 路径（或手工设 `endpoints.overrides.zcode.bin`/自制 shim）。headless 还需要用户自行开通的凭据，**按智谱平台分两路**（z.ai 与 BigModel 同属智谱——国际版与国内开放平台，账号体系各自独立）：(a) **z.ai（智谱国际版）**账号 → `node <安装目录>/resources/glm/zcode.cjs login`（Z.AI OAuth；无浏览器加 `--no-browser`）；(b) **BigModel（智谱国内开放平台）**Coding Plan（apiKey+baseURL 指向 bigmodel.cn）→ 把桌面端 `~/.zcode/v2/config.json` 的 `provider` 块手工同步进 `~/.zcode/cli/config.json`（或直接写入），或用 env 三件套指向 bigmodel 端点。注意 `zcode login` **只覆盖 z.ai 国际版**，开不出国内 BigModel 凭据。`doctor` 以 `credential_ready` 预检。 |
