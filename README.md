...
| zcode | – / – / ✅ | yolo-only by design (non-yolo tiers wait forever headless). Desktop installs no PATH binary — the default install dirs are auto-probed via `detect.known_paths`; for a custom install location the init wizard offers an in-wizard bin-path intake (or set `endpoints.overrides.zcode.bin` / make a shim by hand). Headless also needs credentials the user provisions: the official way is `node <install-dir>/resources/glm/zcode.cjs login` (Z.AI OAuth; add `--no-browser` where no browser exists) to create the shared login — alternatives: the env triple (`ZCODE_MODEL`/`ZCODE_BASE_URL`/`ANTHROPIC_API_KEY`), or hand-copying the desktop's `provider` block from `~/.zcode/v2/config.json` into `~/.zcode/cli/config.json` (what existing desktop users do). `doctor` prechecks this as `credential_ready`. |
