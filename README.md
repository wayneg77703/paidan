...
| zcode | – / – / ✅ | yolo-only by design (non-yolo tiers wait forever headless). Desktop installs no PATH binary — the default install dirs are auto-probed via `detect.known_paths`; for a custom install location the init wizard offers an in-wizard bin-path intake (or set `endpoints.overrides.zcode.bin` / make a shim by hand). Headless also needs credentials paidan never touches: `doctor` prechecks them (`credential_ready`) and points at the native `~/.zcode/cli/config.json` model section or the env triple. |
