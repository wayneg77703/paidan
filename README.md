...
| zcode | – / – / ✅ | yolo-only by design (non-yolo tiers wait forever headless). Desktop installs no PATH binary — the default install dirs are auto-probed via `detect.known_paths`; for a custom install location the init wizard offers an in-wizard bin-path intake (or set `endpoints.overrides.zcode.bin` / make a shim by hand). Headless also needs credentials the user provisions: **copy the `provider` section from `~/.zcode/v2/config.json` into the `provider` section of `~/.zcode/cli/config.json`** (the v2 file holds only that one section — merge it in; cli's own `hooks`/`plugins`/`mcp`/`model` keys are untouched). That is what makes the endpoint callable for existing desktop users (BigModel/GLM plans, z.ai, or other providers the desktop is signed into). z.ai (Zhipu international) accounts can alternatively use `node <install-dir>/resources/glm/zcode.cjs login` (Z.AI OAuth; `--no-browser` where headless), and `ZCODE_MODEL`/`ZCODE_BASE_URL`/`ANTHROPIC_API_KEY` work as an env fallback. `doctor` prechecks this as `credential_ready`. |
