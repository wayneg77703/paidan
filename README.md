...
| zcode | – / – / ✅ | yolo-only by design (non-yolo tiers wait forever headless). Desktop installs no PATH binary — the default install dirs are auto-probed via `detect.known_paths`; for a custom install location the init wizard offers an in-wizard bin-path intake (or set `endpoints.overrides.zcode.bin` / make a shim by hand). Headless also needs credentials the user provisions, **by account type**: (a) Z.AI's own account → `node <install-dir>/resources/glm/zcode.cjs login` (Z.AI OAuth; `--no-browser` where headless); (b) a third-party Anthropic-compatible provider like the BigModel/GLM coding plan (`apiKey`+`baseURL`) → hand-copy the desktop's `provider` block from `~/.zcode/v2/config.json` into `~/.zcode/cli/config.json` (or write it there directly), or use the env triple (`ZCODE_MODEL`/`ZCODE_BASE_URL`/`ANTHROPIC_API_KEY`) pointed at that provider. `zcode login` only covers (a) — it does NOT provision (b). `doctor` prechecks this as `credential_ready`. |
